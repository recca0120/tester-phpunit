...
import { junitParser, generateEmpty } from './junit-parser';
...
export function run(textEditor :?TextEditor, additionalArgs: ?string) {
  return new Promise((resolve) => {
    ...
    const xmlFilePath = `${tmpdir()}/junit-${(new Date().getTime())}.xml`;
    ...
    const exit = async () => {
      ...
      const results = parseReporterOutput(
        processOutput,
        filePath,
        existsSync(xmlFilePath) === true
          ? await readFileAsync(xmlFilePath, 'utf8')
          : generateEmpty(filePath),
        );

      await unlinkAsync(configurationFilePath);
      await unlinkAsync(xmlFilePath);
