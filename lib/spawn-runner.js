...
function getPHPBinary(binary) {
  return (binary === '' && fs.existsSync(binary) === true) ? binary : 'php';
}

function getPHPUnitBinary(projectPath, binary) {
  if (binary === '') {
...
export function run(textEditor/* :TextEditor*/) {
  return new Promise((resolve) => {
    ...
    const args = _.union([getPHPUnitBinary(projectPath, atom.config.get('tester-phpunit.phpunitBinaryPath')), outputFilePath, `--log-junit=${junitXML}`], atom.config.get('tester-phpunit.args'));
    const options = { cwd };
    /*
