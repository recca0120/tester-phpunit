...
export function run(textEditor/* :TextEditor*/) {
  return new Promise((resolve) => {
    ...
    const [command, firstArgument] = getCommand(projectPath, atom.config.get('tester-phpunit.phpBinaryPath'), atom.config.get('tester-phpunit.phpunitBinaryPath'));
    const args = _.union([firstArgument, outputFilePath, `--log-junit=${junitXML}`], atom.config.get('tester-phpunit.args'));
    const options = { cwd };
    /*
