...
const unlinkAsync = filePath => new Promise((resolve) => {
...
function getCommand(projectPath :string, phpBinary :string, phpunitBinary :string) {
  if (phpBinary === '') {
    phpBinary = 'php';
  }
...
export function getConfigurationFile(projectPath :string, phpunitConfigurationFilePath :string) {
  ...
  writeFileSync(phpunitConfigurationFilePath, configurationString, 'utf8');

  return phpunitConfigurationFilePath;
}
...
export async function parseReporterOutput(outputString :string, filePath :string, junitXML :string) {
...
export function run(textEditor :?TextEditor, additionalArgs: ?string) {
  return new Promise((resolve) => {
    ...
    const phpunitConfigurationFilePath = `${tmpdir()}/phpunit-${(new Date().getTime())}.xml`;
    ...
    const defaultArgs = [
      ...
      `--configuration=${getConfigurationFile(projectPath, phpunitConfigurationFilePath)}`,
    ];
    if (filePath) {
    ...
    const exit = async () => {
      ...
      let junitXML = '';
      try {
        junitXML = await readFileAsync(junitXMLFilePath, 'utf8');
        await unlinkAsync(phpunitConfigurationFilePath);
