...
export function run(textEditor) {
  ...
  return new Promise((resolve, reject) => {
    ...
    const args = ['--colors', outputFilePath];
    const options = { cwd };
    ...
    const stdout = data => output += data.toString();
    const stderr = data => output += data.toString();
    const exit = (code) => {
      console.log('OUTPUT', output);
      if (code === 0) {
        return resolve({ messages, output });
      }
      return reject({ messages, output });
    };
    ...
    process.onWillThrowError((errorObject) => {
      ...
      return reject({ messages, output: "Couldn't find mocha binary." });
    });
  });
}
