...
export function junitParser(xml :string) :Array<Object> {
  return parseTestsuite(xml2json(xml).testsuites);
}

export function generateEmpty(filePath :string) :string {
  let className = filePath.replace(/\\/g, '/');
  className = className.substr(className.lastIndexOf('/') + 1).replace(/\.php$/i, '');
